...
URL_CLASIFICACION_PRIMERA = 'https://www.eduardolosilla.es/quiniela/ayudas/clasificacion'

URL_CLASIFICACION_SEGUNDA = 'https://www.bdfutbol.com/es/t/t2020-212a.html'

URL_RACHAS = 'https://www.eduardolosilla.es/quiniela/ayudas/rachas'

URL_PRESUPUESTOS_PRIMERA = 'https://www.resultados-futbol.com/primera/grupo1/equipos'

URL_PRESUPUESTOS_SEGUNDA = 'https://www.resultados-futbol.com/segunda/grupo1/equipos'
...
def get_clasificacion():
    ...
    clasificacion = {}
    html_page = requests.get(URL_CLASIFICACION_PRIMERA)
    ...
    html_page = requests.get(URL_CLASIFICACION_SEGUNDA)
    soup = BeautifulSoup(html_page.content, features='html.parser')
    # Buscamos la tabla html de la clasificacion
    tabla = soup.find('table', {'class': 'taula_estil sortable taula_classificacio'})
    ...
    for fila in tabla:
        if fila.name is not None:
            ...
            columnas = fila.find_all('td')
            if columnas:
                posicion = int(NUM.match(columnas[1].text).group(0))
                equipo = normalize(NOMBRE.search(columnas[3].text).group(0)).upper()
                puntos = int(NUM.match(columnas[4].text).group(0))
                ganados = int(NUM.match(columnas[6].text).group(0))
                empatados = int(NUM.match(columnas[7].text).group(0))
                perdidos = int(NUM.match(columnas[8].text).group(0))
                golesFavor = int(NUM.match(columnas[9].text).group(0))
                golesContra = int(NUM.match(columnas[10].text).group(0))
                clasificacion[equipo] = {'posicion': posicion,
                                         'puntos': puntos,
                                         'p_ganados': ganados,
                                         'p_empatados': empatados,
...
def get_presupuestos():
    ...
    html_page = urllib.request.urlopen(URL_PRESUPUESTOS_PRIMERA).read().decode('utf-8')
    ...
    html_page = urllib.request.urlopen(URL_PRESUPUESTOS_SEGUNDA).read().decode('utf-8')
    soup = BeautifulSoup(html_page, features='html.parser')
